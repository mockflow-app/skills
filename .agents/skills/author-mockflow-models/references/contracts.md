...
```json
{
  ...
  "delivery": {
    ...
    "retry_owner_ref": "<current retry-owner component_ref>"
  },
  "target": {
...
Call `get_contract_draft` after commit and `get_contract_gap_report`. Do not report contract completion while data usages are unsynchronized, schema references are missing, or blocking gaps remain.
